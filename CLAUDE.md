...
- DB schema 迁移**只**在 `packages/core/storage/alembic/versions/`（当前 `0001…0025`，单一 head `0025_drop_broll_overlay`；`0023`/`0024`/`0025` 是 contract 字段删除后的 JSONB 清理迁移；`0014` 合并过早期双 `0012` 分支，两个 `0018` 文件是线性顺接、非分叉）。
