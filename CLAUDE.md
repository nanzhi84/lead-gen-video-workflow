...
- DB schema 迁移**只**在 `packages/core/storage/alembic/versions/`（当前 `0001…0021`，单一 head `0021_voice_vendor_status`；`0012` 从 `0011` 分叉出 `case_rubric` / `selection_ledger_clip_id` 两支、由 merge revision `0014` 合并；`0018` 两个同号文件是线性顺接、非分叉）。
