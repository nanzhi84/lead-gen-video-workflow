...
- 浏览器上传走 `/api/uploads/prepare` → object-store presigned PUT → `/api/uploads/complete`；API 不代理文件字节，complete 阶段只做 HEAD/sha256/content-type/媒体探测与产物登记。
...
- `LocalObjectStore` 支持 presign 只是 dev/test 替身；生产直传上传需 `CUTAGENT_OBJECTSTORE_BACKEND=s3`/OSS，并先跑 bucket CORS/lifecycle provisioning。
