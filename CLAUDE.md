...
- DB schema 迁移**只**在 `packages/core/storage/alembic/versions/`（当前 `0001…0021`，单一 head `0021_voice_vendor_status`；其中 `0012`（`case_rubric` 与 `selection_ledger_clip_id` 同源并行）、`0018` 各有两支同号文件，`0014` 为 merge revision）。
