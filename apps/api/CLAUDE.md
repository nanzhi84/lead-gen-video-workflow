...
- `services/` — 业务逻辑(约 30 个模块);`media_processing.py`(编排 ffmpeg 稳像/剪辑 + 标注时长对账)、`case_agent_llm.py`(case agent 脚本生成)、`publish_login.py`(发布登录,驱动小V猫 CDP)、`publishing_nodes.py`(发布节点编排)
