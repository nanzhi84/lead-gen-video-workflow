...
- `src/routes.ts` / `src/App.tsx` — 路由表与 router（`/login`、`/register`、`/`、`/studio/:caseId/{profile,agent,outputs,publish}`、`/library/*`、`/analytics/*`、`/account/*`、`/settings`、`/ops/prompts`、`/publish-ops`(发布运维页 `PublishOpsPage`)）。`/studio/:caseId/{runs,finished-videos}` 实为重定向到 `outputs` 的兼容路由；`/publish-center*`、`/ops/*` 同样是重定向兼容路由。
...
- `src/contracts/*.typecheck.ts` — 仅编译期的 API 调用面守卫，落在 `src/` 下，会被 build 步骤 `tsc -b && vite build` 的类型检查覆盖，不在运行时执行。
