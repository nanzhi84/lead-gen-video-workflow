...
按域组织的 pytest 套件（约 146 个文件）。默认只跑不依赖外部基础设施的单测；DB / Temporal 集成测试通过 env flag 显式 opt-in。

## 布局
- 按域分目录：`api` `core` `creative` `media` `planning` `production` `publishing` `ops` `observability` `providers` `prompts` `connectors` `import` `scripts` `workflow`。
...
- `integration/` — `test_sqlalchemy_*.py`，SQLAlchemy 后端集成（需 Postgres）。
- `temporal/` — Temporal 运行时集成。
- `fixtures/` — 共享夹具（`fixtures/media.py` 的 `MediaFixtureFactory` 等）。

## 关键文件
- `conftest.py` — 全局夹具与默认 env：`CUTAGENT_STORAGE_BACKEND=memory`、`CUTAGENT_DISABLE_BACKGROUND_DISPATCHER=1`、`CUTAGENT_ALLOW_SANDBOX_FALLBACK=1`（让 golden/fallback 夹具走 seeded sandbox），并把对象存储指向临时目录；内含进程内 ASGI 测试客户端 `_ASGISyncTestClient`。
...
## 注意 / 坑
- `CUTAGENT_RUN_DB_TESTS` 由 `tests/integration/test_sqlalchemy_*.py` 读取，`CUTAGENT_RUN_TEMPORAL_TESTS` 由 `tests/temporal/test_temporal_runtime.py` 读取；未置则这些用例 skip 而非失败。
