...
def workflow_run_row_to_contract(row: WorkflowRunRow) -> WorkflowRun:
    return WorkflowRun(
        ...
        retry_from_run_id=row.retry_from_run_id,
...
def node_run_row_to_contract(row: NodeRunRow) -> NodeRun:
    return NodeRun(
        ...
        degradations=list(row.degradations or []),
        started_at=row.started_at,
        finished_at=row.finished_at,
        schema_version=row.schema_version,
        created_at=row.created_at,
...
class SqlAlchemyProductionRepository:
    ...
    def _job_row(self, job: Job) -> JobRow:
        return JobRow(
            ...
            request_schema=f"{job.type.value}.request.v1",
    ...
    def _node_run_row(self, node_run: NodeRun) -> NodeRunRow:
        return NodeRunRow(
            ...
            degradations=[
                item.value if hasattr(item, "value") else str(item) for item in node_run.degradations
            ],
    ...
    def _provider_invocation_row(self, invocation: ProviderInvocation) -> ProviderInvocationRow:
        return ProviderInvocationRow(
            ...
            media_seconds=invocation.media_seconds,
            estimated_cost=invocation.estimated_cost.model_dump(mode="json"),
    ...
    def _usage_meter_record_row(self, usage: UsageMeterRecord) -> UsageMeterRecordRow:
        return UsageMeterRecordRow(
            ...
            actual_cost=usage.actual_cost.model_dump(mode="json") if usage.actual_cost else None,
            cost_unpriced=usage.cost_unpriced,
