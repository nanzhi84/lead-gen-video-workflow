...
- `src/routes.ts` / `src/App.tsx` — 路由表与 router（`/login`、`/register`、`/`、`/studio/:caseId/{profile,agent,outputs,publish}`、`/library/*`、`/analytics/*`、`/account/*`、`/settings`、`/ops/prompts`、`/publish-ops`(发布运维页 `PublishOpsPage`)）。其中 `/studio/:caseId/{runs,finished-videos}`（重定向到 `outputs`）与 `/ops/*` 均为重定向兼容路由。
