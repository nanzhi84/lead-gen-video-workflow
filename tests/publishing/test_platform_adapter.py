"""PublishPlatformAdapter port: CDP default + sandbox behavior."""
...
def test_resolve_adapter_id_defaults_to_xiaovmao(monkeypatch):
    monkeypatch.delenv("CUTAGENT_PUBLISH_ADAPTER", raising=False)
    assert resolve_adapter_id() == XIAOVMAO_ADAPTER_ID
...
def test_select_adapter_defaults_to_xiaovmao(monkeypatch):
    monkeypatch.delenv("CUTAGENT_PUBLISH_ADAPTER", raising=False)
...
def test_select_adapter_can_select_sandbox_explicitly():
    adapter = select_adapter(SANDBOX_ADAPTER_ID)
