...
- 数据统计页包含网络诊断 tab，轮询公开的 `/api/health/network` 显示 Postgres/Redis/OSS/Temporal 分段状态。

## 关键文件 / 子目录
- `src/api/client.ts` — `fetchJson`（`credentials: "include"` 带 cookie）+ `api.*` 业务端点封装；错误码映射为中文文案，提取 `request_id`；上传相关的 `sha256Hex`/`putToOss` 只做裸 XHR PUT（无 cookie、无 FormData）；`api.health.network()` 是手写类型，因为 `/api/health/network` 不进 OpenAPI。
...
- `src/components/analytics/NetworkDiagnosticsPanel.tsx` / `analyticsModel.ts` — 网络诊断 tab；`/api/health/network` 返回 503 也会渲染诊断 body，不当作 fetch 失败。
...
- `src/hooks/useRunEvents.ts` — 跑批实时事件，走 `/ws` WebSocket（连接地址取后端返回的 `stream_url`，dev 下对应 vite 的 `/ws` 代理）；重连时带最后一个 `event_id` 作为 `?after=` cursor，避免全量 replay。
...
- 运行事件列表用 `event_id` 去重；heartbeat 只保活，不进入 UI 事件数组。

## 测试
- 前端专项 pytest 在 `tests/frontend`；UI 类型/构建门禁靠 `npm run build`（`tsc -b && vite build`）。CI 见 `.github/workflows/ci.yml` 的 frontend job。
